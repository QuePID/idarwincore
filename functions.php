...
function export_table_structure($table_name, $conn) {
	...
	$fh_structures = fopen('C:/wamp64/www/idarwincore/admin/create_db_structures.sql', "a+");
...
function idarwincore_delete_indexes($myindexes, $row_color, $conn) {
	...
	$delresults = mysqli_query($conn, $delindexquery)  or die (' MySQL Error (Function - iidarwincore_delete_indexes): ' . mysqli_error($conn));
