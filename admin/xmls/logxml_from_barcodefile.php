...
/*iDarwinCore version 1.1
  ...
  This file (logxml_from_barcodefile.php) reads a list of barcodes
  from a TXT file located in the same folder as this php,
  it then reads this file line by line, then generates a url
  that is checked for integrity and the xml results are good.
...
$file = fopen("url_barcodelist.txt","r");
...
while(! feof($file))
  ...
  $preurl = fgets($file);
  $turl = trim($preurl);
  $urlprefix = 'http://bisque.iplantcollaborative.org/data_service/image?tag_query=filename:';
  $urlsuffix = '*';
  $url = $urlprefix . $turl . $urlsuffix;
